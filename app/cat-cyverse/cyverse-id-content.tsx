...
const SEGMENT_IDS = ['faq', 'manual', 'support'] as const;
export type CatCyverseSegmentId = (typeof SEGMENT_IDS)[number];
...
export function CatCyverseIdContent({ id }: { id: CatCyverseSegmentId }) {
  const { content, title } = getContentAndTitleById(id);
  ...
  return (
    <div className="grid grid-cols-6 gap-x-6 gap-y-3">
      <div className="col-span-full space-y-3 lg:col-span-6">
        ...
        {id === 'faq' && <AccordionFaq data={questionsAndAnswers} />}
      </div>
      {id === 'support' && (
        <>
