    ...
    <div id="accordion-collapse" data-accordion="collapse">
      {data.map((item, index) => (
        ...
        <h2 id={`accordion-collapse-heading-${index}`}>
          <button
            type="button"
            className={`font-base flex w-full items-center justify-between ${index === 0 ? 'rounded-t-lg' : ''} ${index === data.length - 1 ? 'border-b' : 'border-b-0'} border p-5 text-left text-gray-700 hover:bg-custom-isabelline focus:ring-1 focus:ring-custom-isabelline dark:border-gray-700 dark:text-gray-400  dark:focus:ring-gray-800`}
